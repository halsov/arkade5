...
namespace Arkivverket.Arkade.Util
{
    public class InformationPackageCreator
    {
        ...
        {
            string sipDirectory = PrepareSipDirectoriesAndFiles(archive.WorkingDirectory);
            ...
            using (var tarOutputStream = new TarOutputStream(outStream))
            {
                ...
                foreach (var directory in Directory.GetDirectories(sipDirectory))
                {
        ...
        private string PrepareSipDirectoriesAndFiles(DirectoryInfo archiveWorkingDirectory)
        {
            string sipDirectory = archiveWorkingDirectory + "sip";
            Directory.CreateDirectory(sipDirectory + "\\content");

            DirectoryInfo descriptiveMetadata = Directory.CreateDirectory(sipDirectory + "\\descriptive_metadata");
            File.Move(archiveWorkingDirectory + "", );


            DirectoryInfo administrativeMetadata = Directory.CreateDirectory(sipDirectory + "\\administrative_metadata");
